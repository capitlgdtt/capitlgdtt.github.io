...
import { fetchPublicTeamMembers } from "../../../services/teamService";
...
const TeamPageSection: React.FC = () => {
    ...
    useEffect(() => {
        const loadMembers = async () => {
            try {
                const data = await fetchPublicTeamMembers(currentLanguage as 'en' | 'ru');
                setMembers(data);
            } catch (err) {
                console.error(err);
            } finally {
                                ...
                                >
                                    <div className="border rounded-none overflow-hidden h-full flex flex-col bg-[var(--bg-secondary)]/30 backdrop-blur-sm">
                                        ...
                                        <div className="p-4 sm:p-6 flex-1 flex flex-col">
                                            <div className="mb-4">
                                                <h3 className="text-2xl font-syne font-semibold mb-2">{member.name}</h3>
                                                <p className="text-[var(--accent)] font-medium mb-3">{member.role}</p>
                                                <p className="text-[var(--text-secondary)] text-sm leading-relaxed">{member.description}</p>
                                            </div>
                                                ...
                                                <div className="flex flex-col space-y-2!">
                                                    ...
                                                    >
                                                        <img
                                                            ...
                                                            style={{ filter: theme === "dark" ? "invert(1) brightness(2)" : "invert(0)" }}
                                                        />
                                                        {member.email}
                                                    </a>
                                                    <a
